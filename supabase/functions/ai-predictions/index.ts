...
serve(async (req) => {
  ...
  try {
    ...
    // Rate limiting
    const authUserId = req.headers.get('authorization')?.replace('Bearer ', '') || userId || '';
    const ip = req.headers.get('x-forwarded-for') || 'unknown';
    const rateLimit = checkRateLimit(authUserId, ip);
    
    if (!rateLimit.allowed) {
      const retryAfter = Math.ceil((rateLimit.resetTime - Date.now()) / 1000);
      console.log(`AI Stock Intelligence [${requestId}]: Rate limited - retry after ${retryAfter}s`);

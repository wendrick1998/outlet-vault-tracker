-- Migração para unificação do sistema de estoque
-- Primeiro, vamos migrar dados da tabela inventory para stock_items
...
SELECT 
    ...
    CASE 
        ...
        WHEN i.status = 'loaned' THEN 'emprestado'::stock_status
...
RETURNS TRIGGER AS $$
BEGIN
    -- Atualizar status do stock_item baseado no loan
    IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
        UPDATE public.stock_items 
        SET 
            status = CASE 
                WHEN NEW.status = 'active' THEN 'emprestado'::stock_status
                WHEN NEW.status = 'returned' THEN 'disponivel'::stock_status 
                WHEN NEW.status = 'sold' THEN 'vendido'::stock_status
                WHEN NEW.status = 'lost' THEN 'perdido'::stock_status
                ELSE status
            END,
            updated_at = now()
        ...
        SET 
            status = CASE 
                ...
                WHEN NEW.status = 'lost' THEN 'lost'::inventory_status
                ELSE status
            END,
            updated_at = now()
    ...
    IF TG_OP = 'DELETE' THEN
        -- Quando empréstimo é deletado, marcar como disponível
        UPDATE public.stock_items 
        SET 
...
-- Função para sincronizar quando inventory é atualizado diretamente  
...
BEGIN
    IF TG_OP = 'UPDATE' THEN
        ...
        SET 
            status = CASE 
                ...
                WHEN NEW.status = 'loaned' THEN 'emprestado'::stock_status
                WHEN NEW.status = 'sold' THEN 'vendido'::stock_status
    ...
    IF TG_OP = 'INSERT' THEN
        -- Inserir novo item no stock quando criado no inventory
        INSERT INTO public.stock_items (
            inventory_id, imei, serial_number, brand, model, color, storage,
            condition, battery_pct, notes, status, location
        ) VALUES (
            NEW.id, NEW.imei, NEW.serial_number, NEW.brand, NEW.model, NEW.color, NEW.storage,
            NEW.condition, NEW.battery_pct, NEW.notes,
            CASE 
                WHEN NEW.status = 'available' THEN 'disponivel'::stock_status
                WHEN NEW.status = 'loaned' THEN 'emprestado'::stock_status
                WHEN NEW.status = 'sold' THEN 'vendido'::stock_status

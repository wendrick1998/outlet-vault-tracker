-- Create purpose-based secure customer access functions with granular access control and audit logging

-- Drop existing function to recreate with better security
DROP FUNCTION IF EXISTS public.get_customers_secure();

-- Create enum for access purposes to control data exposure
CREATE TYPE customer_access_purpose AS ENUM (
  'general_view',      -- Basic viewing - masks sensitive data
  'loan_processing',   -- Creating/managing loans - needs phone/contact info
  'administrative',    -- Admin tasks - full access with audit
  'search_only'        -- Search functionality - minimal data
);

-- Enhanced secure customer data access with purpose-based controls
...
-- Secure search function that doesn't expose sensitive data in search results
...
-- Enhanced list function with purpose-based access
...
-- Create secure registered customers function

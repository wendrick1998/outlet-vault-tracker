-- FASE 1: EXPANSÃO DE SEGURANÇA E ROLES

-- 1. Expandir enum de roles para incluir 'auditor'
...
-- 9. Função para validar horário de trabalho
...
DECLARE
    ...
    current_day INTEGER;
BEGIN
    -- Pegar horários do usuário
    SELECT turno, horario_inicio, horario_fim 
    INTO user_turno, user_inicio, user_fim
    FROM public.profiles 
    WHERE id = user_id;
    
    -- Se não tem turno definido, permite acesso
    IF user_turno IS NULL THEN
        RETURN true;
    END IF;
    
    current_time := CURRENT_TIME;
    current_day := EXTRACT(DOW FROM CURRENT_DATE); -- 0=domingo, 1=segunda, etc.
    
    -- Não permite acesso nos fins de semana (exceto integral)
    IF current_day IN (0, 6) AND user_turno != 'integral' THEN
        RETURN false;
    END IF;
    
    -- Verificar horário conforme turno
    CASE user_turno
        WHEN 'manha' THEN
            RETURN current_time BETWEEN '06:00'::TIME AND '14:00'::TIME;
        WHEN 'tarde' THEN
            RETURN current_time BETWEEN '14:00'::TIME AND '22:00'::TIME;
        WHEN 'noite' THEN
            RETURN current_time BETWEEN '22:00'::TIME AND '06:00'::TIME;
        WHEN 'integral' THEN
            RETURN true;
        ELSE
            -- Usar horários personalizados
            RETURN current_time BETWEEN COALESCE(user_inicio, '08:00'::TIME) AND COALESCE(user_fim, '18:00'::TIME);
    END CASE;
END;
$$;
...
-- 12. Trigger para registrar alterações em profiles
...
-- 13. Atualizar políticas de profiles para incluir auditor
DROP POLICY IF EXISTS "Managers can view basic profiles" ON public.profiles;
...
-- 14. Função para validar complexidade de senha (será usada no frontend)

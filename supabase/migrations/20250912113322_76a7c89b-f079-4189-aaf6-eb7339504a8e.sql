-- Phase F3: Structured Reasons System (fixed - without problematic triggers)

-- Create reason categories enum
CREATE TYPE IF NOT EXISTS public.reason_category AS ENUM (
...
CREATE TYPE IF NOT EXISTS public.reason_priority AS ENUM (
    'low',
    'medium', 
    'high',
    'urgent'
...
DROP POLICY IF EXISTS "Admins can manage reason workflows" ON public.reason_workflows;
CREATE POLICY "Admins can manage reason workflows"
    ON public.reason_workflows FOR ALL
    TO authenticated
    USING (get_user_role(auth.uid()) = 'admin'::app_role);

DROP POLICY IF EXISTS "All authenticated users can view reason workflows" ON public.reason_workflows;
...
DROP POLICY IF EXISTS "All authenticated users can view SLA tracking" ON public.sla_tracking;
CREATE POLICY "All authenticated users can view SLA tracking"
    ON public.sla_tracking FOR SELECT
    TO authenticated
    USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "System can manage SLA tracking" ON public.sla_tracking;
...
DROP POLICY IF EXISTS "Users can view their assigned approvals" ON public.movement_approvals;
...
DROP POLICY IF EXISTS "Authorized users can manage approvals" ON public.movement_approvals;
...
DROP TRIGGER IF EXISTS update_sla_tracking_updated_at ON public.sla_tracking;
CREATE TRIGGER update_sla_tracking_updated_at
    BEFORE UPDATE ON public.sla_tracking
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_movement_approvals_updated_at ON public.movement_approvals;

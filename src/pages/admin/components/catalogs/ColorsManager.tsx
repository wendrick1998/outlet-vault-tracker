...
import { useColors, useColorMutations } from "@/hooks/useCatalogs";
...
export const ColorsManager = () => {
  ...
  const { data: colors = [], isLoading } = useColors(includeArchived);
  const { createColor, updateColor, archiveColor } = useColorMutations();
  ...
  const handleArchive = async (id: string) => {
    await archiveColor.mutateAsync(id);
  };
                ...
                <TableCell className="text-right space-x-2">
                  ...
                  {color.is_archived ? (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => handleReactivate(color.id)}
                    >
